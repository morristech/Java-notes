package me.shouheng.data.type.container;
...
public class HashMap<K,V> extends AbstractMap<K,V> implements Map<K,V>, Cloneable, Serializable {
    ...
    // region 静态字段和实例字段
    private static final int DEFAULT_INITIAL_CAPACITY = 1 << 4; // 默认容量，16，是2的整次幂
    private static final int MAXIMUM_CAPACITY = 1 << 30;        // 默认最大容量，必须小于等于2的幂次
    private static final float DEFAULT_LOAD_FACTOR = 0.75f;
    private static final int TREEIFY_THRESHOLD = 8;  // 指定的链表长度大于它就变成树结构的
    private static final int UNTREEIFY_THRESHOLD = 6;
    private static final int MIN_TREEIFY_CAPACITY = 64;

    transient Node<K,V>[] table; // 长度是2的整数次幂
    private transient Set<Map.Entry<K,V>> entrySet;

    transient int size;
    transient int modCount;
    private int threshold; // 下次调整表的大小，值为: capacity*load factor，大于该值就调整哈希表容量
    ...
    // region 静态工具方法
    // 计算哈希码的方法，将key的哈希码的高16位和低16位取异或，目的是加大低位的随机性
    ...
    // 根据传入的cap计算对应的容量，实际返回的是不小于cap的最小的2的整数次幂和最大容量中较小的一个
    ...
    /**
     * TODO 在哈希表中查找指定键和哈希码的结点的方法
     *
     * @param hash 哈希码
     * @param key 键
     * @return 找到的结点，因为树节点继承自LinkedHashMap.Entry，而LinkedHashMap.Entry继承了
     * HashMap.Entry，所以直接返回Node<K,V>类型即可 */
    final Node<K,V> getNode(int hash, Object key) {
        Node<K,V>[] tab; Node<K,V> first, e; int n; K k;
        if ((tab = table) != null && (n = tab.length) > 0 && (first = tab[(n - 1) & hash]) != null) {
            // 上面的if语句的意思是哈希表不为空，并且指定哈希码对应的拉链不为空
            ...
            if ((e = first.next) != null) {
                if (first instanceof TreeNode) {
                    // 如果该拉链的第一个元素是树结点，那么我们调用树结点的方法查找
     ...
     * 向哈希表中添加一个键值对的操作
     *
     * @param hash 指定的结点的哈希码
     * @param key 指定结点的键
     * @param value 指定结点的值
     * @param onlyIfAbsent 如果为true，就不更新指定键的值（当键存在的时候）
     * @param evict if false, the table is in creation mode.
     * @return 之前的键值对的值，如果之前不存在就返回null */
    private V putVal(int hash, K key, V value, boolean onlyIfAbsent, boolean evict) {
        ...
        if (p == null) { // 指定的桶位是空的
            ...
            if (p.hash == hash && ((k = p.key) == key || (key != null && key.equals(k)))) {
                // 指定桶的第一个结点恰好是我们要插入的结点
                e = p;
            } else if (p instanceof TreeNode) {
                // 插入到树结构中
                e = ((TreeNode<K,V>)p).putTreeVal(this, tab, hash, key, value);
            } else {
                // 启用循环来查找，判断新插入的结点是否已经存在，如果不存在就插入一个，否则对其进行更新
                // 除了插入的逻辑，这里还包含了将链表变成树的相关的逻辑：如果链表长度大于等于7就变成树结构
                for (int binCount = 0; ; ++binCount) {
                    if ((e = p.next) == null) {
                        // 说明没有找到相同键的结点，那么将新结点插入到最后
                        p.next = newNode(hash, key, value, null);
                        if (binCount >= TREEIFY_THRESHOLD - 1) {
                            // 看来如果某个链表的长度大于等于7就要变成树结构了
                            treeifyBin(tab, hash);
                        }
                        break;
                    }
            ...
            if (e != null) { // 如果e不为null就说明指定的键是存在的
                V oldValue = e.value;
        ...
        // 如果插入了新的结点之后，哈希表的容量大于了指定的尺寸就调整它的大小
        if (++size > threshold) {
            resize();
        }
    ...
    private void treeifyBin(Node<K,V>[] tab, int hash) {
        ...
        if (tab == null || (n = tab.length) < MIN_TREEIFY_CAPACITY) {
            ...
            // 下面的代码实现的逻辑是将链表转换成树
            TreeNode<K,V> hd = null, tl = null;// hd是树的头结点
            do {
                // 获取一个对应于指定结点的树结点
    ...
    public boolean containsValue(Object value) {
        Node<K,V>[] tab; V v;
        if ((tab = table) != null && size > 0) {
            for (Node<K, V> aTab : tab) {
                // 查找指定的值，要将所有的结点全部遍历一遍……
                for (Node<K, V> e = aTab; e != null; e = e.next) {
